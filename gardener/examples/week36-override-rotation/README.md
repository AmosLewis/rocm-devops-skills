...
| [`rocm-systems#9738`](https://github.com/ROCm/rocm-systems/pull/9738) | approval (PARTIAL) | 2 code owners outstanding (xiaogang-chen-amd, dayatsin-amd); build failure is the stale `kBlockNameMap` break in rdc (fixed on develop), diff is `kfdtest` tests only |
