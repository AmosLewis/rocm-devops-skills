...
| 2 | **Re-check, don't override** — review flipped to APPROVED but still CI-BLOCKED | rocm-systems #10085 | Was PARTIAL approval; now `reviewDecision=APPROVED`. Confirm with `check_approval.py`, then it's a CI question, not an approval one — required gates still failing, drill/rebase |
| 3 | **Rebase/re-run, do not bypass** — failures are the *stale* `kBlockNameMap` build break | rocm-systems #10844, #8690 | The break was fixed on develop (#10947 revert + #10973 reland). Neither diff touches rdc/amdsmi. Fresh CI on current develop clears them. #8690 is also STACKED (child #9632) → merge bottom-up. **Hard rule bars bypassing a compile failure.** |
| 4 | **Route to CODEOWNERS** | rocm-systems #9431 | `REVIEW_REQUIRED`, zero failing checks — never a bypass |
| 5 | **Rebase onto fixed develop, do not bypass** | rocm-systems #9738 | Real Linux `kBlockNameMap` compile break (#10179 rerun) + not approved |
| 6 | **Drill/rebase, do not bypass** | rocm-libraries #10233 | Required `Math CI Summary` genuinely FAILURE + coverage-floor; 6-day stale run; not infra-proven |
| 7 | **Watch, no per-PR action** — subrepo mirror `patch does not apply` | #10083, #10653 (sys), #10311 (lib) | Repo-level mirror drift (`Generate and apply patches` / `Update Repositories in the Monorepo`). Not recoverable by re-run; owner is the mirror workflow, not the PR author |
...
| [rocm-systems#8690](https://github.com/ROCm/rocm-systems/pull/8690) | Hui, **stacked** (child #9632) | BLOCKED, APPROVED | same stale kBlockNameMap break | rebase + re-run, then merge bottom→top | No (Hard rule) |
...
| [rocm-systems#9738](https://github.com/ROCm/rocm-systems/pull/9738) | SierraGuiza, #10179 rerun | BLOCKED, not approved | **real** Linux kBlockNameMap compile break | rebase onto fixed develop + get review | No (build break) |
...
- rocm-systems manual set: #5618, #5638, #9031, #9584, #9696, #9776, #10000, #10011–#10015, #10083,
  #10084, #10139, #10157, #10179, #10210–#10213, #10219, #10310, #10341, #10359, #10380, #10396,
  #10458, #10487, #10556, #10563, #10564, #10579, #10580, #10606, #10628, #10653, #10680, #10682,
  #10704, #10802, #10803, #10845, #10951, #10953.
- rocm-libraries manual set: #10749, #10825, #10957, #10979, #11211, #11249.
- Note the count is every merge done from the gardener account (admin overrides **and** any
  ...
  the table above. #10179 appears here and is the reverted build-break mistake (see Lessons).
- Top non-gardener mergers (context): rocm-systems — marifamd ×20, atgutier ×20, kuhar ×20;
  rocm-libraries — malcolmroberts ×16, illsilin ×10, KKyang ×10.

## Unmerged-PR statistics

| Repo | Open PRs now (repo-wide) | Open **gardener requests** carried into Wk-37 |
| --- | --- | --- |
| ROCm/rocm-systems | 676 | 5 (#10085, #10844, #8690, #9431, #9738) |
| ROCm/rocm-libraries | 601 | 1 (#10233) |
| **Total** | **1277** | **6** |
...
| --- | --- | --- | --- |
| [#10949](https://github.com/ROCm/rocm-systems/issues/10949) | kBlockNameMap build break tracker (from #10179) | TriveniTadapaneni | Closed by reland #10973 |
| [#10947](https://github.com/ROCm/rocm-systems/pull/10947) | Revert of #10179 | Chiranjeevi Pattigidi | Landed; unblocked develop |
...
| **`kBlockNameMap` `static_assert` in rdc_tests/test_common.cc** | **CODE (build break)** — never bypass | No — rebase onto fixed develop (#10973) |
...
1. **A failing *build* step is never "unrelated" on the requester's/author's word.** #10179 was merged on
   "those are script run failures, unrelated"; it was a real `kBlockNameMap` compile break, broke the
   RDC build on develop, was reverted (#10947) and relanded-with-fix (#10973). Open the build log,
   read the first error line. This produced the process-merge-override **Hard rule**.
2. **A stale build break masquerades as "your PR is broken."** #8690/#10844 inherited the *fixed*
   kBlockNameMap failure from a broken base — rebase/re-run, don't bypass, don't route to owners.
3. **`reviewDecision` lies — use `check_approval.py`.** #10085 read PARTIAL then APPROVED as owner
...
5. **Single vs stacked picks the merge tool.** `--admin` for base==develop-and-childless; CDP
   `enqueue_stack` bottom→top for stacks (#10083/#10084 this week). Each no-ops on the other shape.
