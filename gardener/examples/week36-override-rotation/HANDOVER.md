...
| 5 | **Route the 2 outstanding code owners, then re-run - do not bypass** | rocm-systems [#9738](https://github.com/ROCm/rocm-systems/pull/9738) | `check_approval.py` = **PARTIAL** (approved: shwetagkhatri, cfreeamd, kentrussell; still-requested: **xiaogang-chen-amd, dayatsin-amd**) - a bypass is never for unmet review. Its build failure is only the *stale* `kBlockNameMap` break in `projects/rdc` from [#10179](https://github.com/ROCm/rocm-systems/pull/10179) (run predates the [#10973](https://github.com/ROCm/rocm-systems/pull/10973) fix), not caused by this `kfdtest`-only diff, and clears on a re-run against now-fixed develop |
...
| [rocm-systems#9738](https://github.com/ROCm/rocm-systems/pull/9738) | SierraGuiza, `kfdtest` Cu-mask enablement | BLOCKED, **PARTIAL** approval | 2 code owners outstanding (xiaogang-chen-amd, dayatsin-amd); build failure is the *stale* [#10179](https://github.com/ROCm/rocm-systems/pull/10179) kBlockNameMap break in rdc, not this diff | route the 2 owners, then re-run on now-fixed develop | No (PARTIAL review, not a build break) |
