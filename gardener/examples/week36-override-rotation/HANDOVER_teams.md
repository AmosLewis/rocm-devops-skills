...
**Actions**
- **#10085 (sys) — re-run, don't override.** Review flipped PARTIAL→APPROVED. Now a CI question, not approval. Confirm with check_approval.py.
- **#10844 (sys) — rebase/re-run, don't override.** Red = stale kBlockNameMap build break, fixed on develop (#10947 + #10973). Diff untouched by it.
- **#8690 (sys) — rebase/re-run, don't override.** Same stale build break. Also STACKED (child #9632), merge bottom→top.
- **#9431 (sys) — route to CODEOWNERS.** REVIEW_REQUIRED, no failing checks. Never a bypass.
- **#9738 (sys) — rebase onto fixed develop.** Real Linux kBlockNameMap compile break + not approved.
- **#10233 (lib) — drill Math CI / rebase.** Required Math CI Summary FAILURE + coverage floor. Not infra-proven.
...
- **Merged PR to Patch Subrepos** `patch does not apply` on #10083, #10653 (sys) · **Synchronize Subtrees** on #10311 (lib). Owner = mirror workflow, not the author.

**Daily infra — none fixed by re-running**
- **kBlockNameMap static_assert** — real build break; rebase onto develop #10973, never bypass.
...
**Two gotchas**
- A failing BUILD step is never "unrelated" on the author's word — open the log (this is the #10179 lesson → the Hard rule).
